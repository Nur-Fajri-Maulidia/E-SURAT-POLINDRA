...
class OutboxController extends Controller
{
    ...
    public function data(Request $request)
    {
        if (request()->ajax()) {
            ...
            if ($jenis == null) {
                if ($tahun) {
                    $data = Document::whereYear('created_at', $tahun)->with('unit_kerja')
                    ->orderBy('created_at', 'desc')->outboxbyuser()->get();
                } else {
                    $data = Document::with('unit_kerja')->orderBy('created_at', 'desc')->outboxbyuser()->get();
                }
            } elseif ($jenis == !null) {
            if ($tahun) {
                    $data = Document::whereYear('created_at', $tahun)->with('unit_kerja')->orderBy('created_at', 'desc')->outboxbyuser()->where('category_id', $jenis)->get();
                } else {
                    $data = Document::with('unit_kerja')->orderBy('created_at', 'desc')->outboxbyuser()
                    ->where('category_id', $jenis)->get();
                }
            }
